...
public class ClefDrawer {
    private final VectorDrawable clefDrawable;
    private final float left;
    private final float top;
    private final float scaleFactor;

    public ClefDrawer(Context context, int clefType, float left) {
        this.left = left;
        switch (clefType) {
            case 1: // F-clef
    ...
    public void draw(Canvas canvas) {
        ...
        float right = left + (float) (originalWidth * scaleFactor);
        float bottom = top + (float) (originalHeight * scaleFactor);
